...
const MentorEvaluations: React.FC = () => {
  ...
  const [teamTalentIds, setTeamTalentIds] = useState<string[]>([]);
  const [talents, setTalents] = useState<Record<string, BackendTalentResponse>>({});
  ...
  const loadTeam = async (projectId: string) => {
    try {
      setLoading(true);
      const team = await projectTeamService.getByProject(projectId);
      const ids = Array.from(
        new Set(team.map((t) => String(t.talentId || '')).filter(Boolean)),
      );
      setTeamTalentIds(ids);
      if (!selectedTalentId && ids.length) setSelectedTalentId(ids[0]);

      // Best-effort fetch talent details for display.
      const missing = ids.filter((id) => !talents[id]);
      if (missing.length) {
        const results = await Promise.all(
          missing.map((id) => talentService.getTalentById(id).catch(() => null)),
        );
        setTalents((prev) => {
          const next = { ...prev };
          results.forEach((t) => {
            if (t?.id) next[String(t.id)] = t;
          });
          return next;
        });
      }
    } catch (e: any) {
      toast.error(e?.message || 'Failed to load project team');
      setTeamTalentIds([]);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      await evaluationService.createEvaluation({
        ...
        dto: {
          ...
          evaluatedType: 'TALENT',
          evaluatedId: String(selectedTalentId),
  ...
  const selectedTalentLabel = useMemo(() => {
    const t = talents[selectedTalentId];
    if (!t) return selectedTalentId || '—';
    return `${t.studentCode || 'Talent'} (${t.id})`;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Mentor evaluates talents in their projects.</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {teamTalentIds.map((id) => (
                    <option key={id} value={id}>
                      {talents[id]?.studentCode ? `${talents[id].studentCode} (${id})` : id}
                    </option>
                  ))}
                </select>
                <div className="mt-1 text-xs text-gray-500">Selected: {selectedTalentLabel}</div>
              </div>
              <div className="text-sm text-gray-600">
                <div>Project status: <StatusBadge status={String(selectedProject?.status || '—')} /></div>
                  <div className="mt-1 text-xs text-gray-500">Evaluation allowed when status is COMPLETED.</div>
                <div>Team size: {teamTalentIds.length}</div>
              </div>
            </div>
          </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={onSubmit}
                  disabled={loading || !selectedProjectId || !selectedTalentId || selectedProject?.status !== 'COMPLETED'}
                />
              </div>
            </div>
          </Card>
